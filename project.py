...
game = 1
before_game = pygame.image.load("deltarune.webp")
before_game = pygame.transform.scale(before_game, (500,500))
window.blit(before_game, (0,0))
#работа со скриптом
text = '''Нажми на союзника (слева) после нажатия'''
...
knopka_kr = Kartinka(100, 100, "kris_back.png", 80, 120)
...
while game:
    ...
    for iw in pygame.event.get():
        ...
        if iw.type == pygame.MOUSEBUTTONDOWN:
            ...
            if knopka_spm.visible and knopka_spm.rect.collidepoint(x, y):
                knopka_spm.visible = False
                pygame.mixer.music.load("attack_slash.mp3")
                pygame.mixer.music.set_volume(0.4)
                pygame.mixer.music.play(0)

            if knopka_jevil.visible and knopka_jevil.rect.collidepoint(x, y):
                knopka_jevil.visible = False
                pygame.mixer.music.load("attack_slash.mp3")
                pygame.mixer.music.set_volume(0.4)
                pygame.mixer.music.play(0)
            if knopka_tenna.visible and knopka_tenna.rect.collidepoint(x, y):
        ...
        if iw.type == pygame.MOUSEBUTTONDOWN:
            ...
            if knopka_kng.visible and knopka_kng.rect.collidepoint(x, y):
